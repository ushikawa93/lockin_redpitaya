...
(* DONT_TOUCH = "true" *) reg [6:0] switch_reg;
...
assign bram_portc_rst = switch_reg[0] == 1 ? bram_porta_rst : bram_portb_rst;
assign bram_portc_addr = switch_reg[1] == 1 ? bram_porta_addr : bram_portb_addr;
assign bram_portc_wrdata = switch_reg[2] == 1 ? bram_porta_wrdata : bram_portb_wrdata;
assign bram_portc_we = switch_reg[3] == 1 ? bram_porta_we: bram_portb_we;

assign bram_porta_rddata = switch_reg[4] == 1 ? bram_portc_rddata : {(BRAM_DATA_WIDTH){1'b0}};
assign bram_portb_rddata = switch_reg[5] == 0 ? bram_portc_rddata : {(BRAM_DATA_WIDTH){1'b0}};
...
BUFGMUX #(
   ...
   BUFGMUX_inst (
      ...
      .S(switch_reg[6])    // 1-bit input: Clock select
   );
